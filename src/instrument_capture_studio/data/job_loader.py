...
class LoadedCaptureJob:
    """Reload one Capture Job from disk."""
...
def load_capture_job(job_directory: Path) -> LoadedCaptureJob:
    """Load the current formal schema-v1 recipe data."""
    ...
    if recipe == "ext_imm_pair":
        ...
        context = CaptureContext(
            ...
            spectrum_freerun=_load_spectrum(
                job_directory,
                "spectrum_freerun.npz",
                spectra.get("freerun"),
            ),
            waveform_sync=_load_waveform(
                job_directory,
                "waveform_sync.npz",
                oscilloscope.get("sync"),
            ),
            waveform_followup=_load_waveform(
                job_directory,
                "waveform_followup.npz",
                oscilloscope.get("followup"),
            ),
