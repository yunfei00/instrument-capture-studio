...
class FakeDSOX:
    def acquire_delay_group(self, *, cancel_check=None):
        return (
    ...
    def acquire_cycle_group(self, *, cancel_check=None):
        return (
...
def test_imm_spectrum_only_is_complete_without_dsox():
    ...
    assert result.metadata["acquisition_modes"] == {"fsw_imm": "single"}
    assert fsw.trigger == "IMM"
    assert sink.context.spectrum_imm is not None
    assert sink.context.spectrum_imm.metadata["acquisition_mode"] == "single"
    assert sink.context.waveform_delay is None
    assert sink.context.waveform_cycle is None


def test_dsox_only_captures_two_distinct_single_groups_without_fsw():
    sink = FakeSink()
    ...
    assert sink.context.waveform_delay.metadata["acquisition_command"] == ":SINGle"
    assert sink.context.waveform_cycle.metadata["acquisition_command"] == ":SINGle"
